...
export default class DataChatManager {
    static async run() {
        setInterval(() => {
            ...
            const time = getTime();
            const parentChatTime = originData.chatId
                ?.split("-")
                ?.slice(0, 3)
                ?.join("-");
            ...
            const chatLogData: TChatLogData[] = (() => {
                const saveObject: TChatLogData = {
                    ...{ ...chatObject, chatName: undefined },
                };
                if (!fs.existsSync(chatLogFilePath)) {
                    return [saveObject];
                }

                const originChatData = (getJsonData(chatLogFilePath) ??
                    []) as TChatLogData[];
                if (originData.chatId === undefined) {
                    return [...originChatData, saveObject];
                }

                const parentChat = originChatData.find(
                    (v) => v.chatId === originData.chatId
                );
        ...
        return chattingList
            ...
            .map((v) => {
                if (this.getChatType(v.chatName) === "direct") {
                    v.chatName = v.chatParticipant.filter(
                        (o) => o !== userId
                    )?.[0];
                }
                return v;
