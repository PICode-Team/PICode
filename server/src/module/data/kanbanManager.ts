...
import {
  TkanbanCreateData,
  TkanbanData,
} from "../../types/module/data/kanban.types";
...
export default class DataKanbanManager {
  ...
  static getKanbanInfo(kanbanUUID: string) {
    ...
    return getJsonData(
      this.getKanbanPath(kanbanUUID, "kanbanInfo.json")
    ) as TkanbanData;
  }
  ...
  static updateIssueCount(
    kanbanUUID: string,
    type: "totalIssue" | "doneIssue",
      ...
      .filter((kanban) => {
        const kanbanData =
          kanban !== "milestoneListInfo.json"
            ? this.getKanbanInfo(kanban)
            : undefined;
        return (
          kanban !== "milestoneListInfo.json" &&
          (options.uuid === undefined || options.uuid === kanbanData?.uuid) &&
          (options.column === undefined ||
            (options.column as string) in (kanbanData?.columns as string[])) &&
          (options.projectName === undefined ||
            options.projectName === kanbanData?.projectName)
        );
      })
  ...
  static create(userId: string, kanbanData: TkanbanCreateData) {
    ...
    if (
      !fs
        ...
        .reduce((projectIdList: string[], projectId: string) => {
          projectIdList.push(
            DataProjectManager.getProjectInfo(projectId)?.projectName as string
          );
    ...
    DataAlarmManager.create(userId, {
      ...
      checkAlarm: (
        DataProjectManager.getProjectInfo(
          DataProjectManager.getProjectId(
            userId,
            kanbanData.projectName
          ) as string
  ...
  static update(
    kanbanUUID: string,
    kanbanData: Partial<TkanbanData>,
    userId: string = ""
  ) {
    ...
    if (userId !== "") {
      DataAlarmManager.create(userId, {
        type: "kanban",
        location: "",
        content: `${userId} update ${
          kanbanData.title ?? this.getKanbanInfo(kanbanUUID)?.title
        } kanban at ${kanbanData.projectName}`,
        checkAlarm: (
          DataProjectManager.getProjectInfo(
            DataProjectManager.getProjectId(
              userId,
              kanbanData.projectName ??
                (this.getKanbanInfo(kanbanUUID)?.projectName as string)
            ) as string
          )?.projectParticipants as string[]
        ).reduce((list: { [ket in string]: boolean }, member) => {
          list[member] = true;
  ...
  static delete(userId: string, kanbanUUID: string) {
    if (
      ...
      !fs.readdirSync(this.getKanbanPath()).includes(kanbanUUID)
    ) {
      log.error(
        `[DataKanbanManager] delete -> kanban uuid is not in kanbanList`
      );
      return false;
    }
    const kanbanData = this.getKanbanInfo(kanbanUUID) as TkanbanData;
