...
import { TDockerData, TDockerCreateData, TDockerNetworkJsonData, TDockerNetworkData, TDockerNetworkCreateData, TBridgeInfo, TDockerUpdateData } from "../../types/module/data/docker.types";
...
export default class DataDockerManager {
    ...
    static createNetwork(dockerNetworkInfo: TDockerNetworkCreateData) {
        ...
        this.commandDockerAsync(
            command,
            (networkId: Buffer) => {
                const networkInfo = this.commandDockerSync(`docker network inspect --format="{{".IPAM.Config"}}" ${dockerNetworkInfo.networkName}`).split(" ");
    ...
    static update(userId: string, projectName: string, dockerInfo: TDockerUpdateData) {
        ...
        if (dockerInfo.bridgeName !== undefined && !Object.keys(newDockerInfo.bridgeInfo).includes(dockerInfo.bridgeName) && dockerInfo.connect === true) {
