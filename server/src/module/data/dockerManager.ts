    ...
    this.commandDockerAsync(
      ...
      (networkId: Buffer) => {
        const networkInfo = this.commandDockerSync(
          `docker network inspect --format="{{".IPAM.Config"}} {{".Containers"}}" ${dockerNetworkInfo.networkName}`
        ).split(" ");
...
CMD ["./server-linux", "${socketPort}"]`;
  ...
  static create(
    ...
    if (dockerInfo.linkContainer !== undefined) {
      const linkProjectId = fs
        ...
        .find((projectId) => {
          return (
            (this.getDockerInfo(projectId) as TDockerData).containerName ===
  ...
  static update(
    ...
    if (
      ...
      if (linkProjectId !== undefined) {
        ...
        linkContainerInfo.containers.push(newDockerInfo.containerName),
          this.setDockerInfo(linkProjectId, linkContainerInfo);
      }
    }
    if (
      ...
      log.debug(
        `docker Info : ${dockerInfo.bridgeName} : ${dockerInfo.bridgeAlias}`
      );
      newDockerInfo.bridgeInfo[dockerInfo.bridgeName as string] =
        dockerInfo.bridgeAlias ?? "";
      log.debug(
  ...
  static getDockerVisualizationInfo() {
      ...
      (projectId) => {
        ...
        visualizationInfo.container.push({
          ...
          parent: Object.keys(dockerInfo.bridgeInfo).reduce(
            (networkIdList: string[], bridgeName: string) => {
              networkIdList.push(
                (this.getDockerNetworkInfo() as TDockerNetworkJsonData)[
                  bridgeName
                ].id
        ...
        if (dockerInfo.portInfo !== undefined) {
          Object.keys(dockerInfo.portInfo as TPortMappingData).map(
            (hostPortString: string) => {
              ...
              const containerPort = (dockerInfo.portInfo as TPortMappingData)[
                hostPortString
              ];
              const portInfoIndex = visualizationInfo.port.findIndex(
                (portInfo: TDockerPortVisualData) => {
                  return portInfo.outBound === hostPort;
                }
              );
              if (portInfoIndex !== -1) {
                visualizationInfo.port[portInfoIndex].inBound?.push(
                  containerPort
                );
                dockerInfo.status === "running"
                  ? (visualizationInfo.port[portInfoIndex].onContainer =
                      dockerInfo.containerId)
                  : visualizationInfo.port[
                      portInfoIndex
                    ].connectedContainers?.push(dockerInfo.containerId);
              } else {
                const newPortInfo = {
                  outBound: hostPort,
    ...
    Object.values(this.getDockerNetworkInfo() as TDockerNetworkJsonData).map(
      (networkInfo: TDockerNetworkData) => {
        const newNetworkInfo = {
          ...
          containers: fs
            .readdirSync(DataProjectManager.getProjectDefaultPath())
            .reduce((containerIdList: string[], projectId: string) => {
              const dockerInfo = this.getDockerInfo(projectId) as TDockerData;
