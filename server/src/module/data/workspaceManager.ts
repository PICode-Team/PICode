...
import { TWorkspaceCreateData, TWorkspaceUpdateData, TWorkspaceData } from "../../types/module/data/workspace.type";
import { setJsonData, getJsonData, isExists, removeData, handle, searchWorkspaceFiles, readCodesFromFile, writeCodeToFile, getAllChildren } from "./fileManager";
import DataUploadManager from "./uploadManager";
import fs from "fs";
import simpleGit from "simple-git";
import { zip } from "zip-a-folder";
import { TFile, TFileData, TReturnFileData, TUploadFileLanguageToSize } from "../../types/module/data/file.types";
...
export default class DataWorkspaceManager {
    ...
    static getWorkspaceDataPath(workspaceId: string, type: undefined | "workspaceInfo.json" = undefined) {
        return type !== undefined ? `${DataDirectoryPath}/workspace/${workspaceId}/${type}` : `${DataDirectoryPath}/workspace/${workspaceId}`;
    }

    static getWorkspaceInfo(workspaceId: string) {
        if (!this.isExists(workspaceId, this.getWorkspaceDataPath)) {
            return undefined;
        }

        return getJsonData(this.getWorkspaceDataPath(workspaceId, "workspaceInfo.json")) as TWorkspaceData;
    }
    ...
    static setWorkspaceInfo(workspaceId: string, data: TWorkspaceData | TWorkspaceCreateData | TWorkspaceUpdateData) {
        ...
        return setJsonData(this.getWorkspaceDataPath(workspaceId, "workspaceInfo.json"), data);
    }
    ...
    static create(
        ...
        const func: {
            [key in string]: (workspaceId: string, source: any) => boolean;
        } = {
        ...
        try {
            ...
            if (!func[source.type](workspaceId, source)) {
