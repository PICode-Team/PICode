...
import { TAlarmFullSet, TAlarmSet } from "../../types/module/data/alarm.types";
...
export default class DataAlarmManager {
    ...
    private static getAlarmFileList() {
        return readdirSync(this.getDefaultDataPath()).filter(v => v !== 'alarm_info.json').reverse()
    }
    ...
    private static saveAlarmData(fileName: string, data: TAlarmFullSet[]) {
        writeFileSync(
            `${this.getDefaultDataPath()}/${fileName}`,
            JSON.stringify(data)
        )
    }
    ...
    static get(userId: string) {
        return this.getAllAlarm().filter(v => Object.keys(v.checkAlarm).indexOf(userId) > -1).map(v => {
    ...
    static checkAlarm(userId: string, alarmRoom: string, alarmId: string, alarmStatus: boolean = true) {
        const prevData = this.getAlarmData(alarmRoom)

        this.saveAlarmData(alarmRoom, prevData.map(v => {
            if (v.alarmId === alarmId && (Object.keys(v.checkAlarm).indexOf(userId) > -1)) {
                return { ...v, checkAlarm: { ...v.checkAlarm, userId: alarmStatus } }
            }
            return v
        }))
    }
