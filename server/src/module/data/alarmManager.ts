...
export default class DataAlarmManager {
  ...
  private static getAlarmData(alarmRoom: string) {
    return (
      JSON.parse(
        readFileSync(`${this.getDefaultDataPath()}/${alarmRoom}`).toString()
      ) as TAlarmSet[]
    ).map((v) => ({ ...v, alarmRoom })) as TAlarmFullSet[];
  }
  ...
  static create(
    userId: string,
    alarmData: Omit<TAlarmSet, "userId" | "alarmId">
  ) {
  ...
  static checkAlarm(
    ...
    this.saveAlarmData(
      ...
      prevData.map((v) => {
        if (
          v.alarmId === alarmId &&
          Object.keys(v.checkAlarm).indexOf(userId) > -1
        ) {
          return {
            ...v,
