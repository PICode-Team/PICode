...
export default class DataTerminalManager {
    ...
    static listenToTerminalWorker(
        ...
        worker.send({
            type: "setup",
            setupData: { projectId: projectId, size: size },
        } as TCommandData);
    ...
    static setUpTerminal() {
        process.on("message", (message: TCommandData) => {
            const workerFunc: {
                [key in string]: (message: TCommandData) => void;
            } = {
    ...
    static workerCommand(message: TCommandData) {
        message.command === "exit"
            ? DataTerminalManager.workerExit()
    ...
    static workerSetUp(message: TCommandData) {
        ...
        if (message.setupData === undefined) {
            (<any>process).send({
                type: "setup",
                command: `no setupData from message`,
            } as TCommandData);
            process.exit(0);
        }
        ptyProcess = pty.spawn(shell, [], {
            ...
            cwd: DataProjectManager.getProjectWorkPath(
                message.setupData.projectId
            ),
        });
