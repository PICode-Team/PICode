...
} from "../../types/module/data/milestone.types";
...
export default class DataMilestoneManager {
  ...
  static getMilestoneInfo(milestoneUUID?: string) {
    ...
    const milestoneListData = getJsonData(
      this.getMilestonePath("milestoneListInfo.json")
    );
    return milestoneUUID
      ? (milestoneListData[milestoneUUID] as TMilestoneData)
      : (milestoneListData as TMilestoneJsonData);
  }

  static setMilestoneInfo(
    milestoneUUID: string,
    milestoneData: TMilestoneData
  ) {
      ...
      : Object.values(this.getMilestoneInfo()).filter((mileStoneData) => {
          return (
            (options?.content === undefined ||
              options.content === mileStoneData.content) &&
            (options?.startDate === undefined ||
              options.startDate === mileStoneData.content) &&
            (options?.endDate === undefined ||
              options.endDate === mileStoneData.endDate) &&
            (options?.title === undefined ||
              options.title === mileStoneData.title)
          );
        });
  }
  ...
  static update(
    userId: string,
    milestoneUUID: string,
    milestoneData: TMilestoneUpdateData
  ) {
  ...
  static delete(userId: string, milestoneUUID: string) {
    ...
    if (!Object.keys(milestoneListData).includes(milestoneUUID)) {
      log.error(
        `[DataMilestoneManager] delete -> milestoneUUID is not in ListData`
      );
    }
