  ...
  const stepDiv = [
    <div className={classes.inputBox} key={0}>
      ...
      <CustomTextField
        ...
        error={!validate.pw}
        errorText={"Isn't required password"}
      />
      <CustomTextField
        label="Confirm Password"
        type="password"
        onChange={(e: any) => setConfirmPw(e.target.value)}
        error={passwd !== confirmPw}
            ...
            {userImage !== undefined ? (
              <div>
                <img
                  ...
                  height={"50px"}
                />
                <p style={{ width: "100%", marginTop: "6px" }}>
                  {userImage!.name}
                </p>
              </div>
            ) : (
    ...
    <div className={classes.root}>
      ...
      <div className={classes.loginForm}>
        <div style={{ textAlign: "center" }}>
          <img
            src={"http://localhost:8000/images/picode-7.svg"}
            width={"110px"}
            draggable={false}
          />
        ...
        >
          {step.map((label, idx) => (
            <Step
              key={label}
              className={clsx(
                activeStep >= idx ? classes.activeCir : classes.disableCir
              )}
            >
              <StepLabel
                className={clsx(
                  activeStep >= idx
                    ? classes.stepperTextact
                    : classes.stepperText
                )}
              >
                {label}
        ...
        <div className={classes.buttonGroup}>
          <div
            className={clsx(
              classes.button,
              stpeInfo[activeStep].prev.display
          ...
          <div
            className={clsx(
              classes.button,
              stpeInfo[activeStep].next.display
                ? classes.activeButton
                : classes.disableButton
            )}
            onClick={() => {
