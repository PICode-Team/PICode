...
function createTerminal(userId: string, { workspaceId, size }: { workspaceId: string; size: { cols: number; rows: number } }) {
    ...
    if (terminalWorker === undefined || workspaceId === undefined) {
        getSocket(userId)?.send(makePacket("terminal", "createTerminal", { code: ResponseCode.internalError, message: "fail to create terminal" }));
    } else {
...
function commandTerminal(terminal: { command?: string; id: string }) {
    try {
        ...
        DataTerminalManager.getUserTerminal(terminal.id).socket.send(makePacket("terminal", "commandTerminal", { code: ResponseCode.internalError, message: "fail to command terminal" }));
    }
}
...
function deleteTerminal(terminal: { command?: string; id: string }) {
    try {
        ...
        DataTerminalManager.getUserTerminal(terminal.id).socket.send(makePacket("terminal", "deleteTerminal", { message: "fail to delete terminal" }));
    }
}
